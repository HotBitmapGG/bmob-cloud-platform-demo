...
import com.hotbitmapgg.geekcommunity.activity.ImageBrowserActivity;
...
public class MessageChatAdapter extends BaseListAdapter<BmobMsg>
{
	...
	public MessageChatAdapter(Context context, List<BmobMsg> msgList)
	{
		...
		options = new DisplayImageOptions.Builder().showImageForEmptyUri(R.drawable.app_icon).showImageOnFail(R.drawable.app_icon).resetViewBeforeLoading(true).cacheOnDisc(true).cacheInMemory(true).imageScaleType(ImageScaleType.EXACTLY).bitmapConfig(Bitmap.Config.RGB_565).considerExifParams(true).displayer(new FadeInBitmapDisplayer(300)).build();
	}
	...
	private View createViewByType(BmobMsg message, int position)
	{
		...
		{// 位置类型
			return getItemViewType(position) == TYPE_RECEIVER_LOCATION ? mInflater.inflate(R.layout.item_chat_received_location, null) : mInflater.inflate(R.layout.item_chat_sent_location, null);
		}
		else if (type == BmobConfig.TYPE_VOICE)
		{// 语音类型
			return getItemViewType(position) == TYPE_RECEIVER_VOICE ? mInflater.inflate(R.layout.item_chat_received_voice, null) : mInflater.inflate(R.layout.item_chat_sent_voice, null);
		}
		else
		{// 剩下默认的都是文本
	...
	public View bindView(final int position, View convertView, ViewGroup parent)
	{
		...
		EmojiconTextView tv_message = ViewHolder.get(convertView, R.id.tv_message);
		// 图片
		RoundedImageView iv_picture = ViewHolder.get(convertView, R.id.iv_picture);
		...
		{// 加载头像-为了不每次都加载头像
			ImageLoader.getInstance().displayImage(avatar, iv_avatar, ImageLoadUtil.defaultOptions(), animateFirstListener);
		}
		else
		{
		...
		iv_avatar.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View view)
			{
				...
				if (getItemViewType(position) == TYPE_RECEIVER_TXT || getItemViewType(position) == TYPE_RECEIVER_IMAGE || getItemViewType(position) == TYPE_RECEIVER_LOCATION || getItemViewType(position) == TYPE_RECEIVER_VOICE)
				{
		...
		if (getItemViewType(position) == TYPE_SEND_TXT
				...
				|| getItemViewType(position) == TYPE_SEND_LOCATION || getItemViewType(position) == TYPE_SEND_VOICE)
		{// 只有自己发送的消息才有重发机制
			// 状态描述
...
//						tv_location.setText("我在" + address);
...
//				break;
			case BmobConfig.TYPE_VOICE:// 语音消息
				try
				{
					if (text != null && !text.equals(""))
					{
						...
						{// 发送的消息
							if (item.getStatus() == BmobConfig.STATUS_SEND_RECEIVERED || item.getStatus() == BmobConfig.STATUS_SEND_SUCCESS)
	...
	private void dealWithImage(int position, final ProgressBar progress_load, ImageView iv_fail_resend, TextView tv_send_status, ImageView iv_picture, BmobMsg item)
	{
		...
		{// 发送的消息
			...
			// 为了方便每次都是取本地图片显示
			ImageLoader.getInstance().displayImage(showUrl, iv_picture, ImageLoadUtil.defaultOptions());
		}
		else
		{
			// TODO
			// 设置imageLoad图片设置option
			ImageLoader.getInstance().displayImage(text, iv_picture, ImageLoadUtil.defaultOptions(), new ImageLoadingListener()
			{
