...
public class VideoGalleryAdapter extends BaseAdapter
{
	...
	public VideoGalleryAdapter(Context c, ImageLoader imageLoader)
	{
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		...
		holder.selectLL.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				if (null != mOnItemCheckListener)
				{
					...
					if (!VideoUtils.sImageItems.contains(data.get(position)))
					{
						return;
					}
		...
		try
		{
			...
			String thumbPath = VideoUtils.getThumbPath(mContext, data.get(position).getId());
			...
			{
				ImageLoader.getInstance().displayImage("file:///" + data.get(position).getPath(), aware, VideoUtils.displayOptions());
			}
		} catch (Exception e)
		{
			e.printStackTrace();
