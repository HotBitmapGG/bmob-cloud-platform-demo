package com.hotbitmapgg.geekcommunity.receive;
...
import com.hotbitmapgg.geekcommunity.activity.MainActivity;
 ...
 * @hcc 如果发送自定义格式的消息，使用sendJsonMessage方法来发送Json格式的字符串
 */
public class HomeMessageReceiver extends BroadcastReceiver
{
                    ...
                    if (TextUtils.isEmpty(tag))
                    {
                        ...
                        BmobChatManager.getInstance(context).createReceiveMsg(json, new OnReceiveListener()
                        {
                            ...
                            public void onSuccess(BmobMsg msg)
                            {
                                ...
                                {
                                    boolean isAllow = HomeMsgApplication.getInstance().getSpUtil().isAllowPushNotify();
                        ...
                        } else if (tag.equals(BmobConfig.TAG_ADD_AGREE))
                        {
                            ...
                            BmobUserManager.getInstance(context).addContactAfterAgree(username, new FindListener<BmobChatUser>()
                            {
                                ...
                                public void onSuccess(List<BmobChatUser> bmobChatUsers)
                                {
                                    // TODO Auto-generated method stub
                                    // 保存到内存中
                                    HomeMsgApplication.getInstance().setContactList(CollectionUtils.list2map(BmobDB.create(context).getContactList()));
                                }
                            });
                            // 显示通知
    ...
    public void showMsgNotify(Context context, BmobMsg msg)
    {
        ...
        boolean isAllowVoice = HomeMsgApplication.getInstance().getSpUtil().isAllowVoice();
        boolean isAllowVibrate = HomeMsgApplication.getInstance().getSpUtil().isAllowVibrate();
